...
class ClearingTransactionDraft(BaseModel):
    ...
    ID/Hash Concepts:
    - draft_id: SHA256 hash of (settlement_date + expense_ids). Used to generate
                YNAB's import_id (format: YS-{hash}-{date}). Ensures idempotency
                for the same set of expenses.
    - import_id: Generated by YnabClient, stored in YNAB for deduplication.
                 Not stored in this model.
    """

    draft_id: str  # Deterministic key for YNAB import_id generation
